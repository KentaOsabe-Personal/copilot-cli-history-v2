...
- **Adjacent expectations**: `django-backend-foundation` は Python backend の実行・検証入口を提供する。`api-contract-fixtures` と現行 Rails 実装は互換期待値の参照元である。後続の `django-presenters-contract`、`bigquery-session-repository`、`django-history-api`、`rails-django-parity-validation` は、この spec が返す normalized session と issue 情報を入力として利用する。
