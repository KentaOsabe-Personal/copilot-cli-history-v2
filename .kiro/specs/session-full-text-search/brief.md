...
GitHub Copilot CLI のローカル会話履歴を読み返す利用者は、日付範囲だけでは目的のセッションへ素早く辿り着けない。会話本文、会話 preview、issue message などの断片を覚えていても、現状は一覧を開いて preview と詳細を順に確認する必要がある。
...
利用者が一覧画面で検索語を入力し、保存済みセッションの会話本文、会話 preview、issue code / message に一致するセッションだけを確認できる。検索は日付範囲と併用でき、結果が空の場合や検索条件を解除した場合も既存の read-only 閲覧体験と整合する。
...
- tool call、activity、作業コンテキスト、repository / branch / model は本文検索の既定対象に含めない
